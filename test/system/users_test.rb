...
class UsersTest < ApplicationSystemTestCase
   ...
   test "failed without facebook credentials" do
     OmniAuth.config.test_mode = true
     visit user_facebook_omniauth_authorize_path
     assert_selector "a", text: "Sign up"
   end
   test "failed with invalid facebook credentials" do
     OmniAuth.config.test_mode = true
     OmniAuth.config.mock_auth[:facebook] = :invalid_credentials
     visit user_facebook_omniauth_authorize_path
     assert_selector "a", text: "Sign Up"
   end
   test "success with valid facebook credentials" do
     OmniAuth.config.mock_auth[:facebook] = init_omniauth_data
     visit user_facebook_omniauth_authorize_path
     assert_selector "a", text: "My Profi"
   end
end
