...
class CoursesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should not be access the my_courses if not signed in" do
    get my_courses_path
    assert_response :redirect
    assert_equal flash[:alert], "You need to sign in or sign up before continuing."
  end
  test "should not be able to access my_courses if signed in" do
  ...
  test "should be redirect_to paypal when trying to subscription a course" do
    sign_in(users(:user_two))
    get subscribe_course_path(courses(:two))
    assert_redirected_to  "https://www.sandbox.paypal.com/cgi_bin/webscr?"+courses(:two).paypal_link(users(:user_two))
  end
