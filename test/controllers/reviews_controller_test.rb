...
class ReviewsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should be able to post review if signed in and has the course" do
    ...
    course = courses(:two)
    sign_in(user)
    post course_reviews_path(course, reviews(:one)), params: {review: {star: 5, comment: "test", course_id: course.id, user_id: user.id}}
    assert_response :redirect
    assert_equal flash[:alert], "Access Denied"
  end
  ...
  test "should not be able to delete review if signed in and has not subscribe the course" do
    user = users(:user_two)
    course = courses(:one)
    sign_in(user)
    delete course_review_path(course, reviews(:one))
